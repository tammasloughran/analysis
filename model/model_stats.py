...
def make_pdf(data):
    ...
    plt.figure()
    plt.hist(distribution, bins=50, normed=True)
    plt.vlines(distribution.mean(), 0, 0.25, colors='g')
    plt.vlines(np.percentile(distribution, 5), 0, 0.25, colors='r', linestyles='dashed')
    plt.vlines(np.percentile(distribution, 95), 0, 0.25, colors='r', linestyles='dashed')
    plt.vlines(0, 0, 0.25, colors='k', linestyles='solid')
    plt.xlabel('Days')
    plt.ylabel('Pr.')
    plt.title('Difference of means between groups')
    plt.show()
    plt.close()


def bootstrap(data1, data2, nsamples=10000):
    ...
    for ni in xrange(nsamples):
        dffrnc[ni,...] = random_replace_subsample(data1) - random_replace_subsample(data2)
...
meanmean, lower, upper = bootstrap(elnino['hwf'], modoki['hwf'])
signif = ((meanmean<0)&(upper<0)) | ((meanmean>0)&(lower>0))
plotmaps(meanmean, signif, 'hwf', 'bootstraped_mean_hwf_elninomodoki.eps', 'bwr')