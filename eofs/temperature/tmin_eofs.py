...
def plot_eigenvalues(eigens, errors):
    ...
    plt.savefig('scree.v0.6_tmin.eps')
...
def plot_pcs(pcs, time):
    ...
    plt.xlabel('Date')
    plt.savefig('pcs_0.6_tmin.eps',format='eps')
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == '--reload':
        # Load the t_max data.
        print 'Loading data'
        filename = 'AWAP_TN_1911-2011_0.5deg.nc'
        ...
        t_max.dump('masked_deseasoned_tmin_0.5d')
        lon.dump('lons_0.5d')
        lat.dump('lats_0.5d')
        time.dump('times_0.5d')
    elif sys.argv[1] == '--continue':
        print 'Loading masked and deseasoned data.'
        t_max = load('masked_deseasoned_tmin_0.5d')
        lon = load('lons_0.5d')
        lat = load('lats_0.5d')
    ...
    plot_eofs(eofs, lon, lat, 'Rotated_EOFs_tmin')
    plot_eofs(eofs2, lon, lat, 'EOFs_tmin')
    plot_eofs(eofs_covariance, lon, lat, 'EOFs_Covariance_tmin')
    plot_eofs(eofs_correlation, lon, lat, 'EOFs_Correlation_tmin')
    plot_pcs(pcs, time)
    print 'Done!'
