"""Turns daily 20C reanalysis omega data into 
summer anomalies.
"""
...
#twentyc_dir = '/srv/ccrc/data35/z5032520/20CRv2/vwnd/'
twentyc_dir = '/srv/ccrc/data35/z5032520/20CRv2/omega/'
file_list = glob.glob(twentyc_dir+'omega.????.nc')
ncget = Dataset(file_list[0])
lat = ncget.variables['lat'][:]
lon = ncget.variables['lon'][:]
level = ncget.variables['level'][:]
...
for cyear in range(1961,1991):
    ncdata = Dataset(twentyc_dir+'omega.%s.nc'%(cyear))
    pr_cyear = ncdata.variables['omega'][:,10,...]
...
# Output file
outfile = Dataset("summer_omega_1911-2011.nc",'w')
# Dims
...
omslp = outfile.createVariable("omega",'f',dimensions=('time','lat','lon'))
setattr(omslp,"long_name","Summer Omega")
setattr(omslp,"units","Pa/s")
setattr(omslp,"missing_value",-9.96921e+36)


omslpa = outfile.createVariable("omegaa",'f',dimensions=('time','lat','lon'))
setattr(omslpa,"long_name","Summer Omega Anomaly")
setattr(omslpa,"units","Pa/s")
setattr(omslpa,"missing_value",-9.96921e+36)
