...
file_list = glob.glob(twentyc_dir+'vwnd.10m.????.nc')
...
for cyear in range(1961,1991):
    ncdata = Dataset(twentyc_dir+'vwnd.10m.%s.nc'%(cyear))
    pr_cyear = ncdata.variables['vwnd'][:]
...
# Output file
outfile = Dataset("summer_vwnd_1911-2011.nc",'w')
# Dims
...
omslp = outfile.createVariable("vwnd",'f',dimensions=('time','lat','lon'))
setattr(omslp,"long_name","Summer Mean U Wind")
setattr(omslp,"units","m/s")
#setattr(omslp,"standard_name","mean_sea_level_pressure")
setattr(omslp,"missing_value",-9.96921e+36)
omslpa = outfile.createVariable("vwnda",'f',dimensions=('time','lat','lon'))
